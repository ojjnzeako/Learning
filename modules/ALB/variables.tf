...
variable interval {
  type        = string
  description = "length of time ALB completes health check"
}

variable timeout {
  type        = string
  description = "time, in seconds, during which no response means a failed health check."
}
...
variable unhealthy_threshold {
  type        = string
  description = "number of consecutive unsuccessful health checks required to consider a target unhealthy."
}
